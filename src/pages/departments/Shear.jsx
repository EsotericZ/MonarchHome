...
export const Shear = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const uniq = [...new Set(shearMaterials.data.flatMap(job => job.jobNo.length > 6 ? job.jobNo.split(' ') : job.jobNo))];
  ...
  const handleSave = async () => {
    try {
      await createMaterial(programNo, material, jobNo, 'shear', 'shear')
      setId(0);
      setProgramNo('');
      setMaterial('');
      setJobNo('');
  ...
  return (
    <Box sx={{ width: '100%', textAlign: 'center', overflowY: 'auto', height: '100vh' }}>
      {loading ? (
        <Box>
          <Typography variant='h4' sx={{ fontWeight: 'bold', margin: '16px' }}>Shear</Typography>
...
{/* Add Modal */}
          ...
          <Dialog open={show} onClose={handleClose} fullWidth>
            ...
            <DialogContent>
              ...
              <TextField label='Area' fullWidth defaultValue='Shear' sx={{ mb: 2, mt: 1 }} disabled />
            </DialogContent>
            <DialogActions>

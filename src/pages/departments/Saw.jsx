...
export const Saw = () => {
  ...
  const fetchData = async () => {
    try {
      const [tbrJobs, frJobs, sawMaterials] = await Promise.all([
        getTBRJobs(),
        getFRJobs(),
        getAllSawMaterials()
      ]);
  ...
  const handleSave = async () => {
    try {
      await createMaterial(programNo, material, jobNo, 'saw', 'saw')
      setId(0);
      setProgramNo('');
      setMaterial('');
      setJobNo('');
...
{/* Add Modal */}
          ...
          <Dialog open={show} onClose={handleClose} fullWidth>
            ...
            <DialogContent>
              ...
              <TextField label='Area' fullWidth defaultValue='Saw' sx={{ mb: 2, mt: 1 }} disabled />
            </DialogContent>
            <DialogActions>
